...
| AC12 | **Neither judged figure** — branch coverage nor groundedness — ever appears without agreement **and the other one** beside it; below 0.85 the table marks both **DIAGNOSTIC** and withholds the published values. *(Reworded 31 Aug 2026 for ADR-0012: as written it named groundedness alone, because groundedness was then the headline. The rule always covered the pair; the inversion made the old wording read as though the headline were exempt.)* | **seen red** by stubbing agreement to 0.80 and confirming the table changes; plus a test asserting the two names appear an equal number of times on **every** rendering path |
...
| AC12 | **MET, seen red, in all four states** | Agreement `None` → withheld; 0.80 → withheld naming the floor; 0.85 → published with both companions; partial run → withheld regardless. Three by unit test, the 0.80 case also live via `--stub-agreement`. **Re-verified 31 Aug 2026 after ADR-0012 inverted the pair:** the withheld block now names both figures, `branch coverage` leads the published block (order asserted by test), and the equal-count test closes the path this AC's old wording left describable but unproven. Not yet seen in a live judge run — confessed. |
