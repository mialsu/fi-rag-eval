...
| **Branch coverage** — the answer-layer headline | **withheld** | 125 branches required | **the published answer-layer number when it can be published** (ADR-0012, 31 Aug 2026). Computed and **not published** today: judge–human agreement does not exist yet. Diagnostic value 0.472, and 0.574 with complete retrieval against 0.042 without — this is where all the answer-layer signal is |
| Answer groundedness | **withheld** | 59 branches stated | computed and **not published**, same reason. Its mandatory companion, never printed alone and no longer the headline: diagnostic value 1.000, *saturated*, and identical with and without complete retrieval. This answerer drops a branch rather than stating one it cannot cite, so the number carries no information about this system |
...
| Refusal precision / recall | **0.632 / 0.923** | n=19 / n=13 | measured, **diagnostic and never a headline**: 13 questions cannot support a published figure. Wilson 95%: [0.41, 0.81] and [0.67, 0.99]. Recall moved from 0.857 n=14 because the **set changed** on 31 Aug 2026 — one entry was wrong and was removed — **not because the answerer improved**. Recomputed offline with `make refusals`; no model, no spend |
| Refusal precision, restricted (`precision*`) | **0.857** | n=14, 5 excused | printed **beside** precision and never instead. Same numerator, denominator excluding refusals of questions whose retrieval was incomplete: the answerer refused 5 of 9 such questions and 2 of 41 with complete retrieval (Fisher p=0.0011), so `precision` charges it for *retrieval's* failures. The gap between the two is a fact about retrieval |
