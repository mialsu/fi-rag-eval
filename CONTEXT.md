...
| Published cell | The single cell whose numbers the README quotes. **A recorded decision, not whichever cell scored highest.** | Currently `snowball/0`. Moving it requires re-recording the baseline, and the gate fails if it moves without that. Picking the winner after seeing all 12 results is golden-set leakage in a new costume — but as of N=50 the argument has changed: `lemma-reasm/0` now beats it at **p=0.039** on the paired test, which is evidence rather than a preference. Still the Owner's decision. |
