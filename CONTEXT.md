...
| Zero-overlap miss | A required chunk that was not retrieved and shares **no** stemmed token with the question. It was unreachable, not out-competed. | A morphology failure: what lemmatisation and compound splitting would fix. |
...
| Lexical leakage | The share of a question's stemmed content words that appear verbatim in its own target chunk. | The measurable form of golden-set leakage. High leakage means the question was written from the source text and the harness is easier than the task. Measured per run. |
