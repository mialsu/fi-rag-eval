...
| Judge | The model that scores answer-layer metrics. Always a different model **family** than the one under test. | Its own agreement with hand labels is reported every run. A same-family judge shares the answerer's blind spots and rubber-stamps the fabrications it exists to catch (ADR-0008). |
